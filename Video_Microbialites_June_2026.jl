...
# This agent represents an approximation of microbial mats that can grow on surfaces or general precipitate process.
...
Base.@kwdef mutable struct WorldParameters
    ...
    sidespeed::Int = 1
    #Probability that a precipitate agent grows
    precip_amplitude::Float64 = 0.08
    #This precip_probs is dyanmically update through the simulations if using periodic precipiator activty
    precip_probs::Float64 = 0.8
    #Probability that a precipitating agent is removed at each time step
    remove_probs::Float64 = 0.02
    #Preferences for precipiatate growth directions
...
video_1 = WorldParameters( sediment_amplitude= 200, downspeed= 2, width = 2000,
    height = 2*950, recolonisation_prob =0.99 , period_control_T=2000, precip_amplitude=0.1, stability_dist = 30, attraction_dist = 30, 
...
video_2 = WorldParameters( sediment_amplitude= 200, downspeed= 2, width = 2000,
    height = 2*950, recolonisation_prob =0.99 , period_control_T=2000, precip_amplitude=0.1, stability_dist = 5, attraction_dist = 30, 
sediment_periodic_mode = :max_cos,
    precip_periodic_mode=  :max_cos, precip_weights_down_side_up = [1,1,3], conical_scale= 0,
    distance_metric = :euclidean, #options should be :euclidean, :manhattan, :chebyshev
    initial_solid_setup_type = :baseline,  prevent_overhang = false
...
video_3 = WorldParameters( sediment_amplitude= 200, downspeed= 2, width = 2000,
    height = 2*950, recolonisation_prob =0.99 , period_control_T=2000, precip_amplitude=0.1, stability_dist = 30, attraction_dist = 2, 
sediment_periodic_mode = :max_cos,
    precip_periodic_mode=  :max_cos, precip_weights_down_side_up = [1,1,3], conical_scale= 0,
    distance_metric = :euclidean, #options should be :euclidean, :manhattan, :chebyshev
    initial_solid_setup_type = :baseline,  prevent_overhang = false
